...
N_TRIALS = 5
POP_SIZE = 3           #number of robot instances
POP_PER_INSTANCE = 3   #number of robots per instance
generation_index = 0
...
for i in range(POP_SIZE):
    pop_per_instance=[SethController() for _ in range(POP_PER_INSTANCE)]
    pop.append(pop_per_instance)

## the evolving population (a list of SethControllers)
...
def check_collisions(controllers, robots):
    robot_controller_index = 0
    while robot_controller_index < len(controllers):
        unique_robot = robots[robot_controller_index]
        controller = controllers[robot_controller_index]
        #### check if robot has collided with FOOD, WATER or TRAP and
        #### update batteries, etc.  accordingly
        ## check for FOOD collisions
        for light in unique_robot.lights[EntityTypes.FOOD] :
            if (unique_robot.x - light.x)**2 + (unique_robot.y - light.y)**2 < ENTITY_RADIUS**2 :
                unique_robot.food_b += 20.0*DT
                controller.trial_data['eaten_FOOD_positions'].append( (light.x,light.y) )
                light.x,light.y = random_light_position(unique_robot) ## relocate entity

        ## check for WATER collisions
        for light in unique_robot.lights[EntityTypes.WATER] :
            if (unique_robot.x - light.x)**2 + (unique_robot.y - light.y)**2 < ENTITY_RADIUS**2 :
                unique_robot.water_b += 20.0*DT
                controller.trial_data['eaten_WATER_positions'].append( (light.x,light.y) )
                light.x,light.y = random_light_position(unique_robot) ## relocate entity

        ## check for TRAP collisions                
        for light in unique_robot.lights[EntityTypes.TRAP] :
            if (unique_robot.x - light.x)**2 + (unique_robot.y - light.y)**2 < ENTITY_RADIUS**2 :
                unique_robot.food_b -= 50.0*DT
                unique_robot.water_b -= 50.0*DT
                score = 0.0
                controller.trial_data['eaten_TRAP_positions'].append( (light.x,light.y) )
                light.x,light.y = random_light_position(unique_robot) ## relocate entity
        robot_controller_index+=1

def check_robot_extinction(robots): #list of robots
        ## DEATH -- if either of the batteries reaches 0, the trial is over
        robot_pop=len(robots)
        num_of_dead_robots = 0
        for unique_robot in robots:
            if unique_robot.food_b < 0.0 or unique_robot.water_b < 0.0 :
                unique_robot.food_b = unique_robot.water_b = 0.0
                num_of_dead_robots += 1
        if num_of_dead_robots == robot_pop:
            return True
        else:
            return False
...
def simulate_trial(controllers,trial_index,generating_animation=False) :
    ...
    for entity_type in EntityTypes:
        ...
        for unique_robot in robots:
            for _ in range(n[entity_type]) :
                x,y = random_light_position(unique_robot)
                entity_light = Light(x,y,entity_type)
                unique_robot.add_light(entity_light)
    ...
    ## batteries
    for unique_robot in robots:
        unique_robot.water_b = 1.0 # msmy016
        unique_robot.food_b = 1.0
    ...
    for iteration in range(N_STEPS) :
        ...
        while robot_controller_index < len(controllers):
            ...
            controller.trial_data['water_battery_h'].append(unique_robot.water_b)
            controller.trial_data['food_battery_h'].append(unique_robot.food_b)
        ...
        for unique_robot in robots:
            DRAIN_RATE = 0.2
            unique_robot.water_b = unique_robot.water_b - DT*DRAIN_RATE
            unique_robot.food_b  = unique_robot.food_b - DT*DRAIN_RATE

        # water_b -= (robot.lm**2) * DT * 0.01
        # food_b  -= (robot.rm**2) * DT * 0.01
        for unique_robot in robots:
            score += (unique_robot.water_b * unique_robot.food_b) * DT
        ...
        while robot_controller_index < len(controllers):
            ...
            controller = controllers[robot_controller_index]
            for entity_type in EntityTypes:
                controller.set_sensor_states(entity_type,unique_robot.sensors[entity_type])
            robot_controller_index+=1
        ...
        while robot_controller_index < len(controllers):
            ...
            unique_robot.lm,unique_robot.rm = controller.get_motor_output((unique_robot.food_b,unique_robot.water_b))
            
            unique_robot.calculate_derivative() ## calculate the way that the robot's state it changing
            unique_robot.euler_update(DT=DT)    ## apply that state change (Euler integration step)
            robot_controller_index+=1
            
        check_collisions(controllers,robots)
        if check_robot_extinction(robots):
            break
    ...
    while robot_controller_index < len(controllers):
        ...
        controller.trial_data['uneaten_FOOD_positions']  = [(l.x,l.y) for l in unique_robot.lights[EntityTypes.FOOD]]
        controller.trial_data['uneaten_WATER_positions'] = [(l.x,l.y) for l in unique_robot.lights[EntityTypes.WATER]]
        controller.trial_data['uneaten_TRAP_positions']  = [(l.x,l.y) for l in unique_robot.lights[EntityTypes.TRAP]]
        controller.trial_data['robot'] = unique_robot

        if TEST_GA :
            scores.append(np.mean(controller.genome))
        robot_controller_index+=1
    return scores
